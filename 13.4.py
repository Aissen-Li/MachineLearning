...
def create_data():
    ...
    sc = MinMaxScaler()  # 对数据进行标准化处理
    sc.fit(data)
    data = sc.transform(data)
    data[:, -1] = data[:, -1] * 2 - 1
    test = np.vstack((data[:15], data[50:65]))  # 30个测试样本
    labeled_sample = np.vstack((data[15:20], data[65:70]))  # 10个有标记样本
    unlabeled_sample = np.vstack((data[20:50], data[70:]))  # 60个无标记样本
    return test, labeled_sample, unlabeled_sample


test, labeled, unlabeled = create_data()
clf = svm.SVC(C=1, kernel='linear')
clf.fit(labeled[:, :2], labeled[:, -1])  # 有标记的样本训练SVM
...
fake_label = clf.predict(unlabeled[:, :2])  # 伪标记
...
weight[len(unlabeled):] = Cu  # 样本权重

train_sample = np.vstack((labeled[:, :2], unlabeled[:, :2]))  # 用于训练有标记与无标记样本集合
train_label = np.hstack((labeled[:, -1], fake_label))  # 用于训练的标记集合
unlabeled_id = np.arange(len(unlabeled))

while Cu < Cl:
    clf.fit(train_sample, train_label, sample_weight=weight)
    while True:
        predicted_y = clf.decision_function(unlabeled[:, :2])  # 通过训练得到的预测标记
        real_y = fake_label  # 伪标记，这里为与预测的区分开，写为real_y
        ...
        if epsilon1 > 0 and epsilon2 > 0 and round(epsilon1 + epsilon2, 3) >= 2:
            ...
            break
    Cu = min(2 * Cu, Cl)  # 更新Cu
    weight[len(unlabeled):] = Cu  # 更新样本权重
